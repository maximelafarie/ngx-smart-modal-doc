...
export class NgxSmartModalComponent implements OnInit, OnDestroy {
  ...
  @ViewChildren('nsmContent') private nsmContent: QueryList<ElementRef> | undefined;
  @ViewChildren('nsmDialog') private nsmDialog: QueryList<ElementRef> | undefined;
  @ViewChildren('nsmOverlay') private nsmOverlay: QueryList<ElementRef> | undefined;
  ...
  public targetPlacement() {
    if (!this.nsmDialog.first || !this.nsmContent.first || !this.nsmOverlay.first || !this.target) {
      return;
    }
    ...
    this._renderer.setStyle(this.nsmContent.first.nativeElement, 'top', offsetTop + 'px');
